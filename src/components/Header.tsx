...
export function Header({ title, subtitle }: HeaderProps) {
  const { user, logout, isAdmin } = useNewAuth();
  ...
  const getRoleBadge = () => {
    if (isAdmin) {
      return <span className="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full">Admin</span>;
    }
    return <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">User</span>;
  };
