...
- **Matching/智能任务分配**：时间符合度绝对优先，时间相同时再按目标精准匹配、当前精准匹配和最小能力超配排序
...
1. **资格检查**：员工/账号启用、每项要求有评估、满足目标精准匹配或当前能力达标，并通过角色门槛。
2. **时间符合度**：按任务工作日的 AM 3.5h、PM 4.5h 计算 `availableHours / requiredHours`；部分冲突保留为候选人。
3. **字典序排序**：时间符合度降序 → 目标精准匹配率降序 → 当前精准匹配率降序 → 当前能力超配值升序。
4. **稳定同分规则**：业务指标仍相同时，建议人选优先，最后按姓名排序。
