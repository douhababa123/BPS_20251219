...
Propose a standard `.xlsx` template with employee ID, skill ID, initial current and annual target, with names/module labels for verification. Score fields remain blank in the template; actual Excel layout must be checked when supplied. All supplied rows require known references, unique employee-skill pairs and numeric `0 <= current <= target <= 4`. Missing cells are not zero-filled or silently dropped; any invalid populated row blocks the entire import.

Preview and cancellation write no business records. Confirmation revalidates the same previewed content, explicit year and authorization before atomically creating the immutable baseline. A year/file change requires a fresh preview. Existing annual baselines require explicit replacement confirmation and an expected active-version check; retain prior versions and allow only one active baseline per year. Persist file source, filename, confirmer and confirmation timestamp without treating upload time as the baseline's effective assessment date.
...
- The actual year-start Excel has not yet been provided; verify its layout against the standard template before activating the 2026 baseline. The standard parser and preview are implemented, but no production baseline may be created from an unconfirmed file.
