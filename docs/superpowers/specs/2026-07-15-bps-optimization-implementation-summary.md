...
本文以当前已部署版本为准。需求 2.1–2.5、3.1–3.3、4.1–4.7 均已实现。原主体功能及 2026-07-15 独立审核发现的能力评估卡片/表格 0–4 显示与统一红绿配色修复，均已通过自动化测试、TypeScript、ESLint、生产构建、GitHub Actions 部署和 Jetson 健康检查。
...
### 2.4 任务类型选择 Leave 后保留工程师并锁定无关字段
...
3. 保留已选工程师，“分配工程师”保持可编辑并设为必填；
4. 清空能力区域、能力项、任务状态和备注等无关输入；
5. 除任务类型、分配工程师、开始日期、结束日期和时间槽以外，其余业务字段禁用；
6. 未选择工程师时不能保存 Leave 任务，防止请假落入“未分配”。
...
- `src/pages/Schedule.tsx`：字段禁用、工程师必填及校验处理
...
- 普通用户录入并分配给自己的日程，保留所选业务状态，空状态默认为 `planned`；
- 自录日程不再转换为 `pending_approval`，也不再要求本人重复确认接受；
- 录入人可以修改或删除自己录入的日程；
- 普通用户将任务分配给其他工程师时，仍强制进入 `pending_approval`；
- 智能匹配的“提交申请 → 管理员审批 → 工程师接受”流程保持不变；
- 幂等迁移仅将历史上“自创建+自分配”的待审批记录改为 `planned`，不修改未分配或分配给别人的任务。

#### 主要代码

- `backend/routers/tasks.py`：当前账号与工程师映射、状态分流和自录日程权限；
- `src/lib/scheduleRules.ts`：工程师待确认任务判断；
- `backend/migrations/007_self_schedule_approval.sql`：历史状态修正。
...
2026-07-15 独立审核修复的部署记录：

- 提交：`142ddcab573586b516c2275f8e977072080a40cb`
- [GitHub Actions 运行 29412182616](https://github.com/douhababa123/BPS_20251219/actions/runs/29412182616)
- “代码质量检查”和“部署到 Jetson”均为 `success`
- Jetson `/api/health` 返回 `status: healthy`、`database: connected`

部署后已确认 Jetson 运行上述提交。卡片/表格统一配色、`L0` 和四级刻度仍应结合真实评估数据完成浏览器视觉复验。
